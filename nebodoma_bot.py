...
async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Укажите ваш номер телефона — мастер свяжется с вами для согласования времени:",
        parse_mode="Markdown",
        reply_markup=markup
    )
    return PHONE
...
async def get_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "📍 Укажите ваш адрес или район города — куда выехать мастеру?"
    )
    return ADDRESS
...
async def get_address(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Наш мастер свяжется с вами в течение *30 минут* для согласования удобного времени.\n\n"
        "🏠 *Небо дома* — делаем ваш потолок идеальным!\n\n"
        "_Если хотите записать ещё одну заявку — нажмите кнопку ниже_ 👇",
        parse_mode="Markdown",
        reply_markup=markup
    )
...
def main():
    ...
    conv_handler = ConversationHandler(
        entry_points=[MessageHandler(
            filters.Regex("📐 Записаться на бесплатный замер"), ask_name
        )],
        states={
